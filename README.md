...
app/
  ...
  services/     Planner, BudgetGuard, JobQueue, Executor, PlanService, промпты, вебхуки
...
tests/          204 теста, ни одного сетевого вызова и ни одного списания
...
### Что покрыто тестами (204 шт., все офлайн)
...
- **Досмотр зависших заданий.** Если вебхук не пришёл и опрос упёрся в `POLL_TIMEOUT_SECONDS`,
  шаг остаётся `failed` по таймауту, хотя генерация могла завершиться. Нужен периодический
  сверщик по `generation_id` для шагов без терминального статуса.
