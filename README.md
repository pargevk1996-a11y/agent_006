...
### Посмотреть за минуту, без ключа и без сети

```bash
uv sync
uv run python scripts/demo_mock.py        # весь цикл в терминале: план → отказ → запуск → защита от повтора
uv run uvicorn app.main:app               # http://127.0.0.1:8000 — веб-интерфейс, /docs — Swagger
```

По умолчанию `APP_MODE=mock`: ни токена, ни сетевых вызовов, ни возможности что-то списать.
Планировщик, бюджет-гард, очередь и SQLite при этом работают настоящие — mock подменяет только
саму платформу, отвечая **записанным слепком реального `GET /capabilities`** (47 моделей).

Проверено вживую: в режиме `live` агент сгенерировал картинку за 1.20 ₽, и списание сошлось
с балансом платформы до копейки — см. §7 и §8.
...
app/
  ...
  static/       index.html — веб-интерфейс одним файлом, без внешних ресурсов
...
tests/          270 тестов, ни одного сетевого вызова и ни одного списания
...
uv run uvicorn app.main:app --reload      # сервис
```

Два входа в одно и то же:

| Адрес | Для кого |
|---|---|
| **http://127.0.0.1:8000** | Веб-интерфейс: бриф → цена с объяснением выбора → явное подтверждение → результат. Одна самодостаточная страница, без CDN и сборки |
| http://127.0.0.1:8000/docs | Swagger: полный API-контракт, схемы, «Try it out» |

Интерфейс показывает режим работы бейджем (`демо` / `только оценки` / **`живой режим — тратятся
настоящие деньги`**) и выносит списание в отдельный блок с точной суммой — перепутать режим или
случайно нажать «выполнить» нельзя.
...
### Что покрыто тестами (270 шт., все офлайн)
