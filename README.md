...
flowchart TB
    ...
    subgraph INF["Инфраструктура"]
        ...
        DB[("SQLite<br/>plans / jobs / step_executions<br/>api_idempotency")]
    end
...
app/
  ...
  services/     Planner, BudgetGuard, JobQueue, Executor, Reconciler, MediaService, промпты, вебхуки
  repositories/ SQLite: планы, задания, ledger шагов, ключи повтора, аудит вебхуков
  core/         конфиг, логирование, маскирование секретов, ошибки
  main.py       сборка приложения, middleware, воркеры, обработчики ошибок
tests/          242 теста, ни одного сетевого вызова и ни одного списания
...
#  "queue_depth":0,"executor_workers":2}
...
| `Idempotency-Key` хранит **ответ**, а не только факт запроса | Клиенту, потерявшему соединение, нужен именно ответ первой попытки; «мы это уже видели» ему бесполезно | Тело ответа дублируется в БД; ключи не протухают сами — для долгой работы нужна чистка старых строк |
...
### Что покрыто тестами (242 шт., все офлайн)
...
- **Срок жизни ключей повтора.** `api_idempotency` растёт без ограничений: нужна чистка строк
  старше суток (и/или TTL в ответе), иначе таблица живёт вечно.
...
- **Срок жизни загруженных файлов.** Ссылки из `POST /upload-media` живут 7 дней, и план,
  построенный на протухшей, упадёт при исполнении. Нужно запоминать загрузки и предупреждать
  при планировании, если ссылка близка к истечению.
